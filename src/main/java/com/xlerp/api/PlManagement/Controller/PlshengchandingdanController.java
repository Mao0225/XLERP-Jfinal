...
public class PlshengchandingdanController extends Controller {
    ...
    @ActionKey("/plshengchandingdan/getpageByDepNo")//根据登录用户所属的部门进行查询
    @HttpMethod("GET")
    public void getpageByDepNo() {
        ...
        String ipoNo = getPara("ipoNo");//生产订单号
        String contractNo = getPara("contractNo");//厂内合同号
        String depNo = getPara("depNo");//登录用户所属的部门编号
        ...
        try {
            ...
            Page<Record> page = plshengchandingdanService.paginateBydep (pageNum, pageSz,ipoNo,contractNo,depNo);
            renderJson (Result.success ("查询成功").putData ("page", page));
        } catch (NumberFormatException e) {
            renderJson (Result.badRequest ("页码或每页大小格式错误"));
        }
    ...
    //生产订单关联的产品操作
    @ActionKey("/plshengchandingdan/item/getList")
    @HttpMethod("GET")
    public void getDingdanItemList() {
        String ipoNo = getPara("ipoNo");//生产订单号
        if (ipoNo == null || ipoNo.trim().isEmpty()) {
            renderJson(Result.badRequest("订单号不能为空"));
        }
        try {
            List<Record> itemList = plshengchandingdanService.getDingdanItemByNo(ipoNo);
    ...
    //生产订单关联的产品操作--根据登录用户所属的部门/车间
    @ActionKey("/plshengchandingdan/item/getListByDepNo")
    @HttpMethod("GET")
    public void getDingdanItemListByDepNo() {
        String ipoNo = getPara("ipoNo");//生产订单号
        ...
        try {
            List<Record> itemList = plshengchandingdanService.getDingdanItemByNoAndDepNo(ipoNo,depNo);
            renderJson(Result.success("查询物料列表成功").putData("itemList", itemList));
        } catch (NumberFormatException e) {
            renderJson(Result.badRequest("页码或每页大小格式错误"));
        }
    ...
    @ActionKey("/plshengchandingdan/item/save")
    @HttpMethod("POST")
    public void saveDingdanItem(Pldingdanitem Item) {
        System.out.println("保存物料1"+ Item);
        try {
            boolean success = plshengchandingdanService.saveDingdanItem(Item);
            if (success) {
    ...
    //修改
    @ActionKey("/plshengchandingdan/item/update")
    @HttpMethod("PUT")
    public void updateDingdanItem(Pldingdanitem Item) {
        try {
            boolean success = plshengchandingdanService.updateDingdanItem(Item);
            if (success) {
    ...
    //删除
    @ActionKey("/plshengchandingdan/item/delete")
    @HttpMethod("DELETE")
    public void deleteDingdanItem() {
        String id = getPara("id");
        if (id == null || id.trim().isEmpty()) {
        ...
        try {
            boolean success = plshengchandingdanService.deleteDingdanItem(Integer.parseInt(id.trim()));
            if (success) {
