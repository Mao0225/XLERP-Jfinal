...
/**
 * Service for ClJgbFc 间隔棒辅材数据采集表
 */
public class ClJgbFcService {
    private static final ClJgbFc dao = new ClJgbFc();

    public Page<ClJgbFc> paginate(
            int pageNumber,
            int pageSize,
            String maQuality,
            String matRecheckNo,
        ...
        StringBuilder from = new StringBuilder("from cl_jgb_fc where isdelete = 0");
    ...
    public ClJgbFc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClJgbFc clJgbFc) {
        return clJgbFc.save();
    }

    public boolean update(ClJgbFc clJgbFc) {
        return clJgbFc.update();
    }
