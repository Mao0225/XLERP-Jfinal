...
import com.xlerp.common.model.ClCt;
...
public class CtService {
    // 数据库访问对象，对应cl_ct表
    private static final ClCt dao = new ClCt();

    /**
     * 分页查询锤头数据
     */
    public Page<ClCt> paginate(int pageNumber, int pageSize, String mafactory, String matRecheckNo, String contractNo,
                               String contractName, String material, String type, String status) {
        String select = "select *";
        StringBuilder from = new StringBuilder(" from cl_ct where 1 = 1");
     ...
     * 根据ID查询锤头记录
     */
    public ClCt findById(int id) {
        return dao.findFirst("select * from cl_ct where id = ? ", id);  // 表名改为cl_ct
    }

    /**
     * 保存锤头记录
     */
    public boolean save(ClCt ct) {
        return ct.save();
    }
     ...
     * 更新锤头记录
     */
    public boolean update(ClCt ct) {
        return ct.update();
    }

    /**
     * 根据ID删除锤头记录（修正方法名首字母小写）
     */
    ...
    public boolean updateStatus(String id, String status, String updatePerson) {
        ...
        // 获取记录
        Record record = Db.findById("cl_ct", id);
        if (record == null) {
            return false;
        }
