...
/**
 * Service for ClFzcFc 防振锤辅材数据采集表
 */
public class ClFzcFcService {
    private static final ClFzcFc dao = new ClFzcFc();

    public Page<ClFzcFc> paginate(
            int pageNumber,
            int pageSize,
            String maQuality,
            String matRecheckNo,
        ...
        StringBuilder from = new StringBuilder("from cl_fzc_fc where isdelete = 0");
    ...
    public ClFzcFc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClFzcFc clFzcFc) {
        return clFzcFc.save();
    }

    public boolean update(ClFzcFc clFzcFc) {
        return clFzcFc.update();
    }
