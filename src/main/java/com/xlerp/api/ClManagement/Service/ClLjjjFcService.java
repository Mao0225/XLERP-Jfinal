...
/**
 * Service for ClLjjjFc 连接金具辅材数据采集表
 */
public class ClLjjjFcService {
    private static final ClLjjjFc dao = new ClLjjjFc();

    public Page<ClLjjjFc> paginate(
            int pageNumber,
            int pageSize,
            String maQuality,
            String matRecheckNo,
        ...
        StringBuilder from = new StringBuilder("from cl_ljjj_fc where isdelete = 0");
    ...
    public ClLjjjFc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClLjjjFc clLjjjFc) {
        return clLjjjFc.save();
    }

    public boolean update(ClLjjjFc clLjjjFc) {
        return clLjjjFc.update();
    }
