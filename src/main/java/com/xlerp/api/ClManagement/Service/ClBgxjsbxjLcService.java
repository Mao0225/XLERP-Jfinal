...
/**
 * Service for ClBgxjsbxjLc 并沟线夹设备线夹铝材数据采集表
 */
public class ClBgxjsbxjLcService {
    private static final ClBgxjsbxjLc dao = new ClBgxjsbxjLc();

    public Page<ClBgxjsbxjLc> paginate(
            int pageNumber,
            int pageSize,
            String mafactory,
            String matMaterial,
        ...
        StringBuilder from = new StringBuilder("from cl_bgxjsbxj_lc where isdelete = 0");
        ...
        if (StrKit.notBlank(chemAl)) from.append(" and chemAl like ?");
        if (StrKit.notBlank(chemSi)) from.append(" and chemSi like ?");
        if (StrKit.notBlank(chemFe)) from.append(" and chemFe like ?");
        if (StrKit.notBlank(chemCu)) from.append(" and chemCu like ?");
        if (StrKit.notBlank(chemMg)) from.append(" and chemMg like ?");
        if (StrKit.notBlank(chemMn)) from.append(" and chemMn like ?");
        if (StrKit.notBlank(chemZn)) from.append(" and chemZn like ?");
        if (StrKit.notBlank(chemTi)) from.append(" and chemTi like ?");
        if (StrKit.notBlank(chemCr)) from.append(" and chemCr like ?");
        if (StrKit.notBlank(tensileStrength)) from.append(" and tensileStrength like ?");
        if (StrKit.notBlank(elongation)) from.append(" and elongation like ?");
        ...
        if (StrKit.notBlank(chemAl)) params.add("%" + chemAl + "%");
        if (StrKit.notBlank(chemSi)) params.add("%" + chemSi + "%");
        if (StrKit.notBlank(chemFe)) params.add("%" + chemFe + "%");
        if (StrKit.notBlank(chemCu)) params.add("%" + chemCu + "%");
        if (StrKit.notBlank(chemMg)) params.add("%" + chemMg + "%");
        if (StrKit.notBlank(chemMn)) params.add("%" + chemMn + "%");
        if (StrKit.notBlank(chemZn)) params.add("%" + chemZn + "%");
        if (StrKit.notBlank(chemTi)) params.add("%" + chemTi + "%");
        if (StrKit.notBlank(chemCr)) params.add("%" + chemCr + "%");
        if (StrKit.notBlank(tensileStrength)) params.add("%" + tensileStrength + "%");
        if (StrKit.notBlank(elongation)) params.add("%" + elongation + "%");
    ...
    public ClBgxjsbxjLc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClBgxjsbxjLc clBgxjsbxjLc) {
        return clBgxjsbxjLc.save();
    }

    public boolean update(ClBgxjsbxjLc clBgxjsbxjLc) {
        return clBgxjsbxjLc.update();
    }
