...
import com.xlerp.common.model.ClBxgls;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

public class BxglsService {
    // 数据库访问对象，对应cl_bxgls表
    private static final ClBxgls dao = new ClBxgls();

    /**
     * 分页查询不锈钢螺栓数据
     */
    public Page<ClBxgls> paginate(int pageNumber, int pageSize, String mafactory, String matRecheckNo, String contractNo,
                               String contractName, String material, String type, String status) {
        String select = "select *";
        StringBuilder from = new StringBuilder(" from cl_bxgls where 1 = 1");
     ...
     * 根据ID查询不锈钢螺栓记录
     */
    public ClBxgls findById(int id) {
        return dao.findFirst("select * from cl_bxgls where id = ? ", id);  // 表名改为cl_bxgls
    }

    /**
     * 保存不锈钢螺栓记录
     */
    public boolean save(ClBxgls bxgls) {
        return bxgls.save();
    }
     ...
     * 更新不锈钢螺栓记录
     */
    public boolean update(ClBxgls bxgls) {
        return bxgls.update();
    }

    /**
     * 根据ID删除不锈钢螺栓记录（修正方法名首字母小写）
     */
    ...
    public boolean updateStatus(String id, String status, String updatePerson) {
        ...
        // 获取记录
        Record record = Db.findById("cl_bxgls", id);
        if (record == null) {
            return false;
        }
        ...
        // 更新记录
        return Db.update("cl_bxgls", record);
    }
}
