...
/**
 * Service for ClJyhFc 均压环屏蔽环辅材数据采集表
 */
public class ClJyhFcService {
    private static final ClJyhFc dao = new ClJyhFc();

    public Page<ClJyhFc> paginate(
            int pageNumber,
            int pageSize,
            String maQuality,
            String orderno,
        ...
        StringBuilder from = new StringBuilder("from cl_jyh_fc where isdelete = 0");
    ...
    public ClJyhFc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClJyhFc clJyhFc) {
        return clJyhFc.save();
    }

    public boolean update(ClJyhFc clJyhFc) {
        return clJyhFc.update();
    }
