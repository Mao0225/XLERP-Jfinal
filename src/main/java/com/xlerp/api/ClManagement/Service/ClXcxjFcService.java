...
/**
 * Service for ClXcxjFc related business logic
 */
public class ClXcxjFcService {
    private static final ClXcxjFc dao = new ClXcxjFc();

    /**
     * 分页查询
     */
    public Page<ClXcxjFc> paginate(
            int pageNumber,int pageSize,String maQuality,String matRecheckNo,
            String orderno,
            String mafactory,
            String sampleNumber,
            String testResult,
        ...
        StringBuilder from = new StringBuilder("from cl_xcxj_fc where isdelete = 0");

        if (StrKit.notBlank(maQuality)) from.append(" and maQuality like ?");
        ...
        if (StrKit.notBlank(sampleNumber)) from.append(" and sampleNumber like ?");
        if (StrKit.notBlank(testResult)) from.append(" and testResult like ?");
        ...
        if (StrKit.notBlank(maQuality)) params.add("%" + maQuality + "%");
        ...
        if (StrKit.notBlank(sampleNumber)) params.add("%" + sampleNumber + "%");
        if (StrKit.notBlank(testResult)) params.add("%" + testResult + "%");
    ...
    public ClXcxjFc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClXcxjFc clXcxjFc) {
        return clXcxjFc.save();
    }

    public boolean update(ClXcxjFc clXcxjFc) {
        return clXcxjFc.update();
    }
