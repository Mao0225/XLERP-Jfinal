...
public class XcxjLcService {
    private static final ClXcxjLc dao = new ClXcxjLc();

    private static final String UPLOAD_DIR = "uploads/";

    public Page<ClXcxjLc> paginate(int pageNumber, int pageSize, String mafactory, String matMaterial, String orderno, String matRecheckNo, String leavefactoryDate, String detectionTime, String certificate, String contractNo, String woNo, String ipoNo, String writer, String writeTime) {
        String select = "select *";
        StringBuilder from = new StringBuilder("from cl_xcxj_lc where isdelete = 0");
    ...
    public ClXcxjLc findById(int id) {
        return dao.findById(id);
    }

    public boolean save(ClXcxjLc xcxjLc) {
        return xcxjLc.save();
    }

    public boolean update(ClXcxjLc xcxjLc) {
        return xcxjLc.update();
    }
...
//    public Page<Record> getNoList(int pageNumber, int pageSize, String no) {
//        // 构建SQL查询语句
//        StringBuilder select = new StringBuilder("SELECT no ");
//        StringBuilder from = new StringBuilder("FROM bascontract ");
//        List<Object> params = new ArrayList<>();
//
//        // 添加查询条件（如果有合同号条件）
//        if (no != null && !no.trim().isEmpty()) {
//            from.append("WHERE no LIKE ? ");
//            params.add("%" + no.trim() + "%");
//        }
//
//        // 添加排序（根据需要调整）
//        from.append("ORDER BY no ASC");
//
//        // 执行分页查询
