...
import com.xlerp.api.ClManagement.Service.LjjjGcService;
...
public class LjjjGcController extends Controller {

    private final LjjjGcService ljjjGcService = new LjjjGcService();

    @ActionKey("/clljjjGc/getpage")
    @HttpMethod("GET")
    public void getpage() {
        ...
        String recheckBatchNo = getPara("recheckBatchNo");

        String tensileStrength = getPara("tensileStrength");
        String yieldStrength = getPara("yieldStrength");
        ...
        try {
            ...
            Page page = ljjjGcService.paginate(pageNum, pageSz, mafactory, matMaterial, orderno, matSpec, batch, recheckBatchNo, tensileStrength, yieldStrength, elongation, leavefactoryDate, detectionTime, certificate, contractNo, woNo, ipoNo, writer, writeTime);
    ...
    public void get() {
        ...
        try {
            ClLjjjGc ljjjGc = ljjjGcService.findById(Integer.parseInt(id));
            if (ljjjGc != null) {
                renderJson(Result.success("查询数据成功").putData("ljjjGc", ljjjGc));
            } else {
    ...
    @ActionKey("/ljjjGc/save")
    @HttpMethod("POST")
    public void save(ClLjjjGc ljjjGc) {
        // 校验必填字段

        try {
            boolean success = ljjjGcService.save(ljjjGc);
            if (success) {
                renderJson(Result.success("数据保存成功").putData("ljjjGc", ljjjGc.getId()));
            } else {
    ...
    @ActionKey("/ljjjGc/update")
    @HttpMethod("PUT")
    public void update(ClLjjjGc ljjjGc) {

        try {

            boolean success = ljjjGcService.update(ljjjGc);
            if (success) {
    ...
    public void delete() {
        ...
        try {
            boolean success = ljjjGcService.deleteById(Integer.parseInt(id.trim()));
            if (success) {
    ...
    public void getWoNoList() {
        ...
        try {
            ...
            Page page = ljjjGcService.getWoNoList(pageNum, pageSz, woNo, ipoNo, contractNo);
            renderJson(Result.success("查询成功").putData("page", page));
        } catch (NumberFormatException e) {
            renderJson(Result.badRequest("页码或每页大小格式错误"));
        }
