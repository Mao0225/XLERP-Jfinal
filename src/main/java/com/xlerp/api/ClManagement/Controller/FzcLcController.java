...
import com.xlerp.api.ClManagement.Service.FzcLcService;
import com.xlerp.api.Common.FileUploadUtils;
import com.xlerp.api.Common.HttpMethod;
import com.xlerp.api.Common.Result;
import com.xlerp.common.model.ClFzcLc;

import java.io.File;
import java.util.HashMap;
import java.util.Map;

public class FzcLcController extends Controller {

    private final FzcLcService fzcLcService = new FzcLcService();

    @ActionKey("/clfzcLc/getpage")
    @HttpMethod("GET")
    public void getpage() {
        ...
        try {
            ...
            Page page = fzcLcService.paginate(pageNum, pageSz, mafactory, matMaterial, orderno, matRecheckNo, leavefactoryDate, detectionTime, certificate, contractNo, woNo, ipoNo, writer, writeTime);
    ...
    public void get() {
        ...
        try {
            ClFzcLc fzcLc = fzcLcService.findById(Integer.parseInt(id));
            if (fzcLc != null) {
                renderJson(Result.success("查询数据成功").putData("fzcLc", fzcLc));
            } else {
    ...
    @ActionKey("/fzcLc/save")
    @HttpMethod("POST")
    public void save(ClFzcLc fzcLc) {
        // 校验必填字段

        try {
            boolean success = fzcLcService.save(fzcLc);
            if (success) {
                renderJson(Result.success("数据保存成功").putData("fzcLc", fzcLc.getId()));
            } else {
    ...
    @ActionKey("/fzcLc/update")
    @HttpMethod("PUT")
    public void update(ClFzcLc fzcLc) {

        try {

            boolean success = fzcLcService.update(fzcLc);
            if (success) {
    ...
    public void delete() {
        ...
        try {
            boolean success = fzcLcService.deleteById(Integer.parseInt(id.trim()));
            if (success) {
    ...
    public void getWoNoList() {
        ...
        try {
            ...
            Page page = fzcLcService.getWoNoList(pageNum, pageSz, woNo, ipoNo, contractNo);
            renderJson(Result.success("查询成功").putData("page", page));
        } catch (NumberFormatException e) {
            renderJson(Result.badRequest("页码或每页大小格式错误"));
        }
