...
import com.xlerp.api.ClManagement.Service.JxjjLcService;
...
public class JxjjLcController extends Controller {

    private final JxjjLcService jxjjLcService = new JxjjLcService();

    @ActionKey("/cljxjjLc/getpage")
    @HttpMethod("GET")
    public void getpage() {
        ...
        try {
            ...
            Page page = jxjjLcService.paginate(pageNum, pageSz, mafactory, maQuality, matMaterial, orderno, matRecheckNo, tensileStrength, elongation, hardness, leavefactoryDate, detectionTime, certificate, contractNo, woNo, ipoNo, writer, writeTime);
    ...
    public void get() {
        ...
        try {
            ClJxjjLc jxjjLc = jxjjLcService.findById(Integer.parseInt(id));
            if (jxjjLc != null) {
                renderJson(Result.success("查询数据成功").putData("jxjjLc", jxjjLc));
            } else {
    ...
    @ActionKey("/jxjjLc/save")
    @HttpMethod("POST")
    public void save(ClJxjjLc jxjjLc) {
        // 校验必填字段

        try {
            boolean success = jxjjLcService.save(jxjjLc);
            if (success) {
                renderJson(Result.success("数据保存成功").putData("jxjjLc", jxjjLc.getId()));
            } else {
    ...
    @ActionKey("/jxjjLc/update")
    @HttpMethod("PUT")
    public void update(ClJxjjLc jxjjLc) {

        try {

            boolean success = jxjjLcService.update(jxjjLc);
            if (success) {
    ...
    public void delete() {
        ...
        try {
            boolean success = jxjjLcService.deleteById(Integer.parseInt(id.trim()));
            if (success) {
    ...
    public void getWoNoList() {
        ...
        try {
            ...
            Page page = jxjjLcService.getWoNoList(pageNum, pageSz, woNo, ipoNo, contractNo);
            renderJson(Result.success("查询成功").putData("page", page));
        } catch (NumberFormatException e) {
            renderJson(Result.badRequest("页码或每页大小格式错误"));
        }
