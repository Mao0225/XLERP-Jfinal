...
import com.xlerp.api.ClManagement.Service.ClFzcFcService;
import com.xlerp.api.PlManagement.Service.PlshengchangongdanService;
import com.xlerp.common.model.ClFzcFc;
import com.xlerp.common.model.Plshengchangongdan;
import com.jfinal.plugin.activerecord.Page;

@Before(HttpMethodInterceptor.class)
public class ClFzcFcController extends Controller {
    private final ClFzcFcService clFzcFcService = new ClFzcFcService();
    private final PlshengchangongdanService plshengchangongdanService = new PlshengchangongdanService();

    @ActionKey("/clfzcfc/getpage")
    @HttpMethod("GET")
    public void getpage() {
        ...
        try {
            ...
            Page page = clFzcFcService.paginate(
    ...
    public void get() {
        ...
        try {
            ClFzcFc clFzcFc = clFzcFcService.findById(Integer.parseInt(id));
            if (clFzcFc != null) {
                renderJson(Result.success("查询成功").putData("clFzcFc", clFzcFc));
            } else {
    ...
    @ActionKey("/clfzcfc/save")
    @HttpMethod("POST")
    public void save(ClFzcFc clFzcFc) {
        try {
            fillIpoNoAndContractNo(clFzcFc);
            boolean success = clFzcFcService.save(clFzcFc);
            if (success) {
                renderJson(Result.success("保存成功").putData("clFzcFcId", clFzcFc.getId()));
            } else {
    ...
    @ActionKey("/clfzcfc/update")
    @HttpMethod("PUT")
    public void update(ClFzcFc clFzcFc) {
        try {
            fillIpoNoAndContractNo(clFzcFc);
            boolean success = clFzcFcService.update(clFzcFc);
            if (success) {
    ...
    public void delete() {
        ...
        try {
            boolean success = clFzcFcService.deleteById(Integer.parseInt(id.trim()));
            if (success) {
     ...
     */
    @ActionKey("/clfzcfc/getGongdanByWoNo")
    @HttpMethod("GET")
    ...
    private void fillIpoNoAndContractNo(ClFzcFc clFzcFc) {
        String woNo = clFzcFc.getWoNo();
        if (woNo != null && !woNo.trim().isEmpty()) {
            Plshengchangongdan gd = plshengchangongdanService.findByWoNo(woNo.trim());
            if (gd != null) {
                clFzcFc.setIpoNo(gd.getIpoNo());
                clFzcFc.setContractNo(gd.getContractNo());
            }
        }
    }
}