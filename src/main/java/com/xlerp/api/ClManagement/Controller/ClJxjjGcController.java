...
import com.xlerp.api.ClManagement.Service.ClJxjjGcService;
import com.xlerp.api.PlManagement.Service.PlshengchangongdanService;
import com.xlerp.common.model.ClJxjjGc;
import com.xlerp.common.model.Plshengchangongdan;
import com.jfinal.plugin.activerecord.Page;

@Before(HttpMethodInterceptor.class)
public class ClJxjjGcController extends Controller {
    private final ClJxjjGcService clJxjjGcService = new ClJxjjGcService();
    private final PlshengchangongdanService plshengchangongdanService = new PlshengchangongdanService();

    @ActionKey("/cljxjjgc/getpage")
    @HttpMethod("GET")
    public void getpage() {
        ...
        try {
            ...
            Page page = clJxjjGcService.paginate(
                    pageNum, pageSz, mafactory, batch, matMaterial, orderno, matRecheckNo,
                    chemC, chemSi, chemMn, chemP, chemS, chemCr, chemNi, chemMo, matSpec, leavefactoryDate,
    ...
    public void get() {
        ...
        try {
            ClJxjjGc clJxjjGc = clJxjjGcService.findById(Integer.parseInt(id));
            if (clJxjjGc != null) {
                renderJson(Result.success("查询成功").putData("clJxjjGc", clJxjjGc));
            } else {
    ...
    @ActionKey("/cljxjjgc/save")
    @HttpMethod("POST")
    public void save(ClJxjjGc clJxjjGc) {
        try {
            fillIpoNoAndContractNo(clJxjjGc);
            boolean success = clJxjjGcService.save(clJxjjGc);
            if (success) {
                renderJson(Result.success("保存成功").putData("clJxjjGcId", clJxjjGc.getId()));
            } else {
    ...
    @ActionKey("/cljxjjgc/update")
    @HttpMethod("PUT")
    public void update(ClJxjjGc clJxjjGc) {
        try {
            fillIpoNoAndContractNo(clJxjjGc);
            boolean success = clJxjjGcService.update(clJxjjGc);
            if (success) {
    ...
    public void delete() {
        ...
        try {
            boolean success = clJxjjGcService.deleteById(Integer.parseInt(id.trim()));
            if (success) {
     ...
     */
    @ActionKey("/cljxjjgc/getGongdanByWoNo")
    @HttpMethod("GET")
    ...
    private void fillIpoNoAndContractNo(ClJxjjGc clJxjjGc) {
        String woNo = clJxjjGc.getWoNo();
        if (woNo != null && !woNo.trim().isEmpty()) {
            Plshengchangongdan gd = plshengchangongdanService.findByWoNo(woNo.trim());
            if (gd != null) {
                clJxjjGc.setIpoNo(gd.getIpoNo());
                clJxjjGc.setContractNo(gd.getContractNo());
            }
        }
    }
}