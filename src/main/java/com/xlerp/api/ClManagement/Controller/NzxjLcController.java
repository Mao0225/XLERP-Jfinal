...
import com.xlerp.api.ClManagement.Service.NzxjLcService;
import com.xlerp.api.Common.FileUploadUtils;
import com.xlerp.api.Common.HttpMethod;
import com.xlerp.api.Common.Result;
import com.xlerp.common.model.ClNzxjLc;

import java.io.File;
import java.util.HashMap;
import java.util.Map;

public class NzxjLcController extends Controller {
    private final NzxjLcService nzxjLcService = new NzxjLcService();

    @ActionKey("/clnzxjLc/getpage")
    @HttpMethod("GET")
    public void getpage() {
        ...
        String hardness = getPara("hardness");
        ...
        try {
            ...
            Page page = nzxjLcService.paginate(pageNum, pageSz, mafactory, maQuality, matMaterial, orderno, matRecheckNo, sampleNumber, tensileStrength, elongation, hardness, leavefactoryDate, detectionTime, certificate, contractNo, woNo, ipoNo, writer, writeTime);
    ...
    public void get() {
        ...
        try {
            ClNzxjLc nzxjLc = nzxjLcService.findById(Integer.parseInt(id));
            if (nzxjLc != null) {
                renderJson(Result.success("查询数据成功").putData("nzxjLc", nzxjLc));
            } else {
    ...
    @ActionKey("/nzxjLc/save")
    @HttpMethod("POST")
    public void save(ClNzxjLc nzxjLc) {
        // 校验必填字段

        try {
            boolean success = nzxjLcService.save(nzxjLc);
            if (success) {
                renderJson(Result.success("数据保存成功").putData("nzxjLc", nzxjLc.getId()));
            } else {
    ...
    @ActionKey("/nzxjLc/update")
    @HttpMethod("PUT")
    public void update(ClNzxjLc nzxjLc) {

        try {

            boolean success = nzxjLcService.update(nzxjLc);
            if (success) {
    ...
    public void delete() {
        ...
        try {
            boolean success = nzxjLcService.deleteById(Integer.parseInt(id.trim()));
            if (success) {
    ...
    public void getWoNoList() {
        ...
        try {
            ...
            Page page = nzxjLcService.getWoNoList(pageNum, pageSz, woNo, ipoNo, contractNo);
            renderJson(Result.success("查询成功").putData("page", page));
        } catch (NumberFormatException e) {
            renderJson(Result.badRequest("页码或每页大小格式错误"));
        }
