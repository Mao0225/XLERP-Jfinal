...
public class InspWorkOrderService {
    private static final PlInspWorkOrder dao = new PlInspWorkOrder().dao();


    /**
     * 分页查询检验单列表（可扩展）
     */
    public Page<PlInspWorkOrder> paginate(int pageNumber, int pageSize, String param, String status) {
        String select = "select p.*,bi.no as itemCode,bi.name as itemName,bi.spec as itemSpec,bi.inclass,bi.unit as itemUnit ";
        // 核心：先加 where 1=1，后续条件统一用 and 拼接
        StringBuilder from = new StringBuilder("from pl_insp_work_order p " +
                "left join basitem bi on bi.id = p.itemId" +
                " where 1=1 ");
    ...
    public boolean updateStatusAndRemark(long id, String newStatus, String operator, String remark) {
        // 1. 查询原记录
        com.jfinal.plugin.activerecord.Record order = Db.findById("pl_insp_work_order", id);
        if (order == null) {
            return false;
        }
        ...
        // 2. 准备更新字段
        com.jfinal.plugin.activerecord.Record update = new Record();
        update.set("id", id);
        ...
        switch (newStatus) {
            // === 报检审核 ===
            case "10"://提交报检
                update.set("reportApplyTime", Timestamp.valueOf(LocalDateTime.now()));
                break;
            // === 检验阶段 ===
            case "20": // 开始检验
            ...
            case "31": // 入库完成
                ...
                update.set("inStockFinishTime", Timestamp.valueOf(LocalDateTime.now()));
                break;
            case "32": // 入库拒绝
                update.set("storageReviewer", operator);
        ...
        // 4. 执行更新
        boolean success = Db.update("pl_insp_work_order", update);
    ...
    public PlInspWorkOrder findById(int i) {
